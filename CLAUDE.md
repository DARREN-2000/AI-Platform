...
You are an AI coding agent working in this repository. Before writing or changing code you MUST follow this protocol. The detailed standards live in `.ai/`. This file is a thin router — keep it that way. `CLAUDE.md` and `AGENTS.md` are identical mirrors so every agent (Claude Code, Codex CLI, Cursor, Gemini CLI, Cline, Roo Code, Windsurf, Aider) resolves the same rules.
...
Reference scaffolding lives in `.ai/templates/` (pyproject, Dockerfile, docker-compose, GitHub Actions CI, eval gate, `.env.example`). Copy and adapt per project.
